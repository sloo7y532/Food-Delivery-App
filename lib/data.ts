 ...
 * Static Demo Data
 *
 * This file contains dummy/sample data used for development, testing,
 * and database seeding. In a production environment, this data would
 * typically come from a CMS or be managed by restaurant staff.
 *
 * The data includes:
 * - Food categories for menu organization
 * - Customization options (toppings, sides)
 * - Complete menu items with nutritional info and pricing
 */

/**
 * Demo data structure containing all static food delivery app data
 * Used for seeding the database and providing realistic test content
 */
const dummyData = {
  /**
   * Food categories for organizing menu items
   * Each category represents a type of food offered by the restaurant
   */
  categories: [
   ...
   * Includes both toppings (add to existing items) and sides (standalone additions)
   * Prices are in cents to avoid floating point issues
   */
  customizations: [
    // Additional toppings for burgers, pizzas, etc.
  ...
  /**
   * Complete menu items with detailed information
   * Each item includes pricing, nutritional info, and available customizations
   * Images are external URLs that get uploaded to Appwrite storage during seeding
   */
  menu: [
    {
